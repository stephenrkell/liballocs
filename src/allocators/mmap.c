...
report_problem:
	...
	if (existing_seq)
	{
		...
		if (executable_mapping_sequence_bigalloc
				&& (char*) executable_mapping_sequence_bigalloc->end
					> (char*) existing_seq->begin
				&& (char*) executable_mapping_sequence_bigalloc->begin
					< (char*) existing_seq->end)
		{
			executable_mapping_sequence_bigalloc = NULL; // FIXME: when will it get reinstated?
			__brk_bigalloc = NULL; // FIXME: when will it get reinstated?
		}
...
struct big_allocation *executable_mapping_sequence_bigalloc __attribute__((visibility("hidden")));
...
static void set_executable_mapping_sequence_bigalloc(void *real_end)
{
	...
	if (!executable_mapping_sequence_bigalloc) abort();
}
		...
		for (int i = 0; i < phnum_auxv->a_un.a_val; ++i)
		{
			...
			else if (phdr->p_type == PT_LOAD)
			{
				...
				if (!(phdr->p_flags & PF_X) &&
					(char*) executable_load_addr + phdr->p_vaddr > (char*) executable_data_segment_start_addr)
				{
					executable_data_segment_start_addr = executable_load_addr + phdr->p_vaddr;
				}
			}
		}
...
void __mmap_allocator_notify_brk(void *new_curbrk)
{
	...
	else if ((char*) new_curbrk > (char*) old_end)
	{
		...
		__liballocs_extend_bigalloc(executable_mapping_sequence_bigalloc, seq->end);
