...
let rec supply_syscall_footprint state extents = 
  print_endline "========================================================================== Entering supply_syscall_footprint";
...
let do_one_syscall env num args =
  ...
  | MoreDataNeeded (state, extents) ->
     ...
     | Finished (retval, write_extents) -> retval (* TODO: actually apply writes *)
     | MoreDataNeeded (_, _) -> failwith "supply_syscall_footprint returned more data needed"
...
let main =
  ...
  | Some env -> begin
      (* shouldn't have use anything from ctypes once
     ...
     let filename_bigstring = Bigstring.of_string filename in begin
         printf "*** %s is the filename; %d is the length of the bigstring\n" filename (Bigarray.Array1.dim filename_bigstring);
         let ptr = raw_address_of_ptr (to_voidp (bigarray_start array1 filename_bigstring)) in begin
             printf "*** 0x%nx is the address we got\n" ptr;
             let open_args = [| ptr |] in
             let retval = (do_one_syscall env open_syscall_num open_args) in
             printf "Got retval from open(): %nd\n" retval
           end
       end
    end
